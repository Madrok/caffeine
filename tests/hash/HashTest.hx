import chx.crypto.hash.Md2;
import chx.crypto.hash.Md5;
import chx.crypto.hash.Sha1;
import chx.crypto.hash.Sha256;
...
class Sha1TestFunctions extends haxe.unit.TestCase {
	public function testSha() {
		...
		assertEquals("84983e441c3bd26ebaae4aa1f95129e5e54670f1",
			Sha1
				.encode(Bytes.ofString("abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq"))
				.toHex()
		);
	}
	...
	public function testVectors() {
		...
		for (x in 0...o.messages.length) {
			try {
				assertEquals(o.hashes[x], Sha1
					.encode(Bytes.ofString(o.messages[x]))
					.toHex()
				);
			}
			catch(e:Dynamic) {
	...
	public function testObjSha() {
		assertEquals("a9993e364706816aba3e25717850c26c9cd0d89d", Sha1.objEncode("abc"));
		assertEquals("03cfd743661f07975fa2f1220c5194cbaff48451", Sha1.objEncode("abc\n"));
		assertEquals("84983e441c3bd26ebaae4aa1f95129e5e54670f1",
	...
	public function testStream() {
		...
		assertEquals("a9993e364706816aba3e25717850c26c9cd0d89d", rv);
	}
	#end
}
...
class Md2Test extends haxe.unit.TestCase {
	function test01() {
		assertEquals("8350e5a3e24c153df2275c9f80692773", Md2
			.encode(Bytes.ofString(""))
			.toHex()
		);
	}
	...
	function test02() {
		assertEquals("32ec01ec4a6dac72c0ab96fb34c0b5d1", Md2
			.encode(Bytes.ofString("a"))
			.toHex()
		);
	}

	function test03() {
		assertEquals("da853b0d3f88d99b30283a69e6ded6bb", Md2
			.encode(Bytes.ofString("abc"))
			.toHex()
		);
	}
	...
	function test04() {
		assertEquals("ab4f496bfb2a530b219ff33031fe06b0",
			Md2
				.encode(Bytes.ofString("message digest"))
				.toHex()
		);
	}
	...
	function test05() {
		assertEquals("4e8ddff3650292ab5a4108c3aa47940b",
			Md2
				.encode(Bytes.ofString("abcdefghijklmnopqrstuvwxyz"))
				.toHex()
		);
	}
	...
	function test06() {
		assertEquals("da33def2a42df13975352846c30338cd",
			Md2
				.encode(Bytes.ofString("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789"))
				.toHex()
		);
	}
}
