...
class AesTestFunctions extends haxe.unit.TestCase {
	...
	static var msg = [0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb,
		0xcc, 0xdd, 0xee, 0xff];
	static var key = [0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b,
		0x0c, 0x0d, 0x0e, 0x0f];
	...
	public function testEcbOne() {
		...
		assertEquals(target,
			BytesUtil
				.encodeToBase(e, Constants.DIGITS_HEXL)
				.toString()
				.substr(0, 32)
		);
	...
	public function testCbcLatinEncrypt() {
		...
		assertEquals(latin_cbc.toLowerCase(), BytesUtil.toHex(e, ""));
	}
	...
	static function doTestAes(bits : Int, phrase : Bytes, msg : Bytes, mode) {
		...
		switch(mode) {
			case CBC:
				aes = cast {
					var c = new ModeCBC(a);
					c.iv = BytesUtil.nullBytes(16);
					c;
				}
			case ECB:
				aes = cast new ModeECB(a);
		}
		var enc : Bytes;
		try {
			enc = aes.encrypt(msg);
		...
		try {
			...
			throw(e + " msg: " + msg + " :: msg len " + msg.length + " :: enc length "
				+ enc.length + ":: " + BytesUtil.hexDump(enc) + " :: dec length " + dec.length
				+ " :: " + BytesUtil.hexDump(dec));
		}
		return true;
	}
}
