...
class Socket extends sys.net.Socket {
	...
	public function addSNICertificate(cbServernameMatch : String->Bool, cert : Certificate,
			key : Key) : Void {
	...
	private function buildConfig(server : Bool) : Config {
		var conf = new Config();
		conf.defaults(server ? SSL_IS_SERVER : SSL_IS_CLIENT, SSL_TRANSPORT_STREAM,
			SSL_PRESET_DEFAULT);
		...
		conf.authmode(if(verifyCert)SSL_VERIFY_REQUIRED else if(verifyCert == null)
			SSL_VERIFY_OPTIONAL else SSL_VERIFY_NONE);
		return conf;
	}
}
